...
$sql= mysqli_query($koneksi,"SELECT * FROM tb_arsip, tb_bidang, tb_pengguna
WHERE tb_arsip.bidang =tb_bidang.id_bidang
AND tb_bidang.id_bidang = '$id_bidang'
AND tb_arsip.tempat='Banjarmasin'");

while ($data=$sql->fetch_assoc()) {
?>
   <h3><center><b>LAPORAN DATA PEMINDAHAN LOKASI ARSIP INAKTIF BIDANG"<?php echo $data['nama_bidang'] ?>" </b></center></h3> 
   <?php
break;
}
   ?>
...
<table border="1" width="100%" style="border-collapse: collapse;">
   ...
   <th width="25px" height="50px">Tanggal Pindah</th>
   ...
   <th width="25px" height="50px">Lokasi</th> -->
    ...
    <tbody>
        <?php
        $no = 1;
        $sql = $koneksi ->query("SELECT * FROM tb_arsip, tb_bidang, tb_pengguna
        WHERE tb_arsip.bidang =tb_bidang.id_bidang
        AND tb_bidang.id_bidang = '$id_bidang'
        AND tb_arsip.tempat='Banjarmasin'");
        while ($data=$sql->fetch_assoc()) {
        ...
        ?>
          <tr>
            ...
            <td width="100px" height="50px"><center><?php echo $data['tgl_pindah'];?> </center></td>   
