					...
					<div class="box-body">
						...
						<div class="form-group">
							...
							<select name="id_barang" id="id_barang" class="form-control select2" style="width: 100%;">
								...
								<?php
								$jurusan = $_SESSION['jurusan'];
								$query = "select * from tb_barang WHERE kepemilikan = '$jurusan' AND kategori= 'Bahan'";
								$hasil = mysqli_query($koneksi, $query);
								while ($row = mysqli_fetch_array($hasil)) {
								?>
...
<?php
    ...
    if ($query_simpan){
      ...
      Swal.fire({title: 'Tambah Data Berhasil',text: '',icon: 'success',confirmButtonText: 'OK'
          ...
          if (result.value) {
              window.location = 'index.php?page=penggunaan_bahan';
          }
      })</script>";
      }else{
      echo "<script>
      Swal.fire({title: 'Tambah Data Gagal',text: '',icon: 'error',confirmButtonText: 'OK'
          ...
          if (result.value) {
              window.location = 'index.php?page=add_penggunaan_bahan';
          }
      })</script>";
    }
  }